...
function MesaSelector({ value, onChange, mesas, loading, rol, isEdit, currentMesaId }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
      ...
      {/* Mensaje de validación para clientes */}
      {isRequired && !value && !loading && (
        <div style={{
          ...
          ⚠️ Los clientes deben tener una mesa asignada
        </div>
      )}
...
/* ─── DEFAULTS ───────────────────────────────────────────────── */
const DEFAULTS = { nombre: '', email: '', password: '', rol: 'cliente', iMesaId: null };
...
const UsuariosModal = ({ isOpen, onClose, onSave, usuario }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Validar que los clientes tengan mesa asignada
    if (formData.rol === 'cliente' && !formData.iMesaId) {
      alert('Los clientes deben tener una mesa asignada');
      return;
    }
    ...
    <>
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{ padding: "20px 24px 24px", display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          {/* Mesa asignada */}
          <div>
            <FieldLabel required={formData.rol === 'cliente'}>
              Mesa Asignada {formData.rol === 'cliente' && "(Obligatoria)"}
            </FieldLabel>
            <MesaSelector
