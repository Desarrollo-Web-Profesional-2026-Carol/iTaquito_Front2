...
import {
  ...
  LogOut, MapPin, Utensils, ClipboardList
} from 'lucide-react';
    ...
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ padding: '14px 14px 12px', flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto', paddingTop: '10px', borderTop: `1px solid ${C.border}` }}>
          ...
          <button onClick={handleAdd} disabled={!disponible}
            style={{ background: added ? C.teal : disponible ? C.pink : C.bgAccent, color: disponible ? '#fff' : C.textMuted, border: 'none', borderRadius: '9px', padding: '7px 14px', fontFamily: FONT, fontWeight: '700', fontSize: '12px', cursor: disponible ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: '5px', transition: 'all 0.2s', boxShadow: added ? glow(C.teal, '55') : disponible ? glow(C.pink, '44') : 'none', minWidth: '90px', justifyContent: 'center' }}>
...
function CartFab({ totalItems, totalPrecio, onClick }) {
  ...
  return (
    <button onClick={onClick}
      style={{ position: 'fixed', bottom: '24px', right: '24px', background: C.pink, color: '#fff', border: 'none', borderRadius: '14px', padding: '12px 20px', fontFamily: FONT, fontWeight: '800', fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', boxShadow: `0 8px 24px rgba(0,0,0,0.4), ${glow(C.pink, '66')}`, zIndex: 150, transition: 'transform 0.2s' }}
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Cliente solo ve productos activos
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '28px 24px 120px' }}>
        ...
        <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: '14px', padding: '16px 18px', marginBottom: '24px', display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center' }}>
          ...
          <button onClick={() => setSoloDisp(s => !s)}
            style={{ background: soloDisp ? `${C.teal}18` : 'transparent', border: `1.5px solid ${soloDisp ? C.teal : C.border}`, borderRadius: '9px', padding: '7px 12px', color: soloDisp ? C.teal : C.textMuted, fontFamily: FONT, fontWeight: '700', fontSize: '12px', cursor: 'pointer', transition: 'all 0.18s', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '5px' }}>
