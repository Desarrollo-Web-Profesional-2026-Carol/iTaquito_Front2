...
function MesaCard({ mesaData, onApprovePayment, onLiberateMesa, processingId }) {
  const { mesa, orders, totalMesa, ordersCount, sessionStart } = mesaData;
      ...
      {open && (
        <div style={{ borderTop: `1px solid ${C.border}`, padding: '16px 18px 18px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button
                onClick={() => onApprovePayment(mesa.id, paymentMethod)}
...
const CajeroPanel = () => {
  ...
  const handleApprovePayment = (mesaId, metodoPago) => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          await cajeroService.approvePayment(mesaId, metodoPago);
